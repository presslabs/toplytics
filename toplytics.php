...
//------------------------------------------------------------------------------
// Return the template filename and path. First is searched in the theme 
// directory and then in the plugin directory
...
function toplytics_needs_configuration_message() {
  ...
  if ( toplytics_needs_configuration() )
    add_action( 'admin_notices', create_function( '', "echo '<div class=\"error\"><p>"
      ...
      admin_url( 'options-general.php?page=' . $plugin_page ) ) . "</p></div>';" ) );
}
...
function toplytics_enqueue_script() {
  wp_enqueue_script(
    'toplytics',
    plugins_url( 'js/toplytics.js' , __FILE__ )
  );
}
