...
//------------------------------------------------------------------------------
	static function ga_statistics() { // Loading all that's required
		require_once 'gapi.oauth.class.php'; // The code
		...
		if ($results && $results['_ts'] + 1800 > time()) { return $results; }
	  	...
	  	try {
			...
			foreach ($ranges as $name => $start_date)
			{
			  ...
			  //error_log('               return_values >>> ' . print_r($return_values,true));
...
//------------------------------------------------------------------------------
  ...
  // We have to catch the oauth login data in admin_init so http headers can be added
	function admin_handle_oauth_login_header() {
		if( isset($_POST['SubmitLogin']) && isset($_POST['toplytics_login_type']) && $_POST['toplytics_login_type'] == 'oauth' ) {
...
//------------------------------------------------------------------------------
    ...
    $params['oauth_callback'] = toplytics_get_admin_url('/tools.php') . '?page=toplytics/toplytics.php&oauth_return=true';
    ...
    if(curl_errno($ch))
    {
      ...
      $info_redirect = toplytics_get_admin_url('/tools.php') . '?page=toplytics/toplytics.php&error_message=' . urlencode($error_message);
    ...
    {
      $info_redirect = toplytics_get_admin_url('/tools.php') . '?page=toplytics/toplytics.php&error_message=' . urlencode($oa_response);
      header("Location: " . $info_redirect);
    }
...
//------------------------------------------------------------------------------
    ...
    if($http_code == 200)
    {
      ...
      $info_redirect = toplytics_get_admin_url('/tools.php') . '?page=toplytics/toplytics.php&info_message=' . urlencode('Authenticated!');
      header("Location: " . $info_redirect);
    }
    else
    {
